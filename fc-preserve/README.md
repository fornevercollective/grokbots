...
Native **AppKit** 3-step + inventory desk lives in `app/` and builds `dist/FC-Preserve.app` (`ai.qbitos.fc-preserve`). The desk lists drives, parts, per-device storage bars, grouped device scroller, Finder thumbs, hotpipe bus, tool routes, and the live.ugrad.ai/motion tether. Window is resizable. Selected devices are a session. Flash stays locked.

...
Native **AppKit** 3-step + inventory desk lives in `app/` and builds `dist/FC-Preserve.app` (`ai.qbitos.fc-preserve`). The desk lists drives, parts, per-device storage bars, grouped device scroller, in-app thumbs, hotpipe bus, tool routes, and an in-app live.ugrad.ai/motion WKWebView. ISO write, models, probe, and doctor stay inside the app. Window is resizable. Selected devices are a session. Flash stays locked.
